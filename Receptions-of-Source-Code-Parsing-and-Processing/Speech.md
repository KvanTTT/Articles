...
На моей практике мне приходилось парсить как небольшие DSL, так и полноценные
языки программирования, такие как Java, PHP, PL/SQL, T-SQL, Objective-C.

В этом докладе я решил рассмотреть наиболее интересные случаи и проблемы, с
которыми может столкнуться любой программист, потому что парсеры используются везде.

И так, начнем со всеми известными и любимыми регулярными выражениями? Почему их
не достаточно для всего? Я думаю большая часть аудитории так или иначе догадываются.
И, несмотря на то, что современные регулярки, например в C#, обладают мощностью
контекстно-свободных языков, описывать ими конструкции с определенного этапа
становится очень сложно. <!-- TODO: добавить пример про HTML -->

Другой более продвинутый подход - использовать поток токенов. **Токеном**
обозначается последовательность определенных символов и их тип. Саму эту
последовательность обычно называют **лексемой**. С токенами иметь дело приятнее,
чем с текстом, потому что в этом случае можно сразу фильтровать пробелы,
комментарии и другие незначимые для нас вещи. Более того, сами типы токенов
можно использовать в конечном автомате.

Однако и токенов не всегда бывает достаточно. Например, как правильно распарсить
вложенные скобки или теги в html или цепочку токенов, зависящую от типа первого
токена? Для обработки таких случаев и были разработаны разные алгоритмы и практики
...
Процесс создания токенов из текста называется токенизацией, а создания деревьев
разбора из потока токенов - парсингом. Однако бывают парсеры, без этапа токенизации,
они называются безлексерными.
...
Далее я опишу типы парсеров и расскажу о том, какие из них существуют под C#, т.к.
у нас все-таки .NET митап.

Готовыми либами парсинга являются, например, встроенные движок регулярных выражений,
XML и внешние библиотеки, например хорошо известный JSON.NET, ServiceStack и другие.
Их достоинствами являются хорошее качество, производительность, т.к. они такие
либы написаны для распространенных форматов, опенсорсные и существуют продолжительное
время. Также они имеют и обширный API, позволяющий, например, прописывать
произвольную логику десериализации для каких-то сложных древовидных или графовых структур.
Кстати, о десериализации древовидных структур я еще расскажу в этом докладе.
Однако такие парсеры не получится использовать для менее распространенных,
а тем более своих предметно-ориентированных языков или DSL (domain specific language).
...
генераторов парсеров. Но и в этом случае разрабатывать такие парсеры очень долго
и займет много-много человекочасов. Примером таких парсеров под C# являются Roslyn.
...
**компиляторы компиляторов**. Генераторы парсеров так названы потому что с
помощью них можно сгенерировать парсеры сразу под несколько языков (или рантаймов)
имея только одно описание языка, которое называет **грамматикой**.
Также существуют библиотеки для создания парсеров в рантайме для конкретного
языка, а не на этапе генерации. Такие инструменты называются **комбинаторами
парсеров**.

Именно на генераторах парсеров акцентирован мой доклад.
Далее я опишу наиболее распространенные и полезные генераторы парсеров под C#.

<!-- Расписать про существующие либы:
Nitra -->

## GPLEX

Лексический анализ, функционирующий на конечных автоматах. Данный лексер генерирует
C# код из формата `.lex`.

## Комбинаторы парсеров

Как я уже упоминал, комбинаторы парсеров встроены в сам язык, а значит никакая
генерация не нужна. В некоторых случаях это немного упрощает воркфлоу, т.к. не
нужно его настраивать в CI. Также такие комбинаторы могут быть интегрированы с
IDE разработчика. На практике комбинаторы очень полезны для мелких задачек
парсинга, чуть сложнее чем регулярные выражения. В общем если вы нуждаетесь
в парсере, но не хотите сильно вникать в предметную область, то комбинаторы
парсеров - ваш выбор.
...
Также в для C# есть комбинатор [superpower](https://github.com/datalust/superpower),
представляющий собой что-то среднее между регулярными выражениями и мощными
стандартами индустрии ANTLR. И другие Parseq, Parsley and LanguageExt.Parsec.

## Проблемы при использовании грамматик и генераторов парсеров

Далее я все же хотел углубиться в инструмент, который я больше всего использую
в моей работе, а именно ANTLR. Как уже я говорил, мы используем его как для
парсинга больших языков программирования Java, PHP, PL/SQL, T-SQL, Objective-C,
так и для DSL. Такой DSL мы используем в универсальном движке сопоставления с
шаблонами PT.PM.
...
<!-- Возможно добавить еще интересных примеров -->

## Решение с использованием вставок кода

Возвращаясь к нашему генератору парсеров ANTLR, проблему с отделением ключевых
...
время парсинга. Обособляются фигурными скобками. Разница между действиями и
...
Для собственного DSL такие предикаты используются.

## Контекстно-зависимые конструкции

Все действия, которые не касаются парсинга, рекомендуется осуществлять в
...
Формально говоря, в этом случае парсинг зависит не только от типа токена, но и
от его значения. В примере на данном слайде таким маркером является `HeredocIdentifier`.
...
В данном случае невозможно грамматику переделать так, чтобы она без вставок
...
Зачем вообще разработчики PHP все так усложнили? Удобно, когда строки нормально
выглядят чисто и не содержат захламляющие символы экранирования. Однако это
получается не всегда и по крайней мере надо как-то обозначать маркеры начала и
окончания строк. Крайне маловероятно, что в Heredoc попадется строка с
`HeredocIdentifier`, однако если попадется, то можно просто использовать другой
идентификатор.

## Интерполируемые строки в C#
...
<!-- TODO: расписать про плохое и хорошее решение -->
...
## Связывание скрытых токенов с нетерминальными узлами (Swiftify)
...
## Связывание скрытых токенов со значимыи токенами (Roslyn)
...
# Препроцессорные директивы: одноэтапная обработка
...
Эту задачу можно решить разными способами: с использованем регулярных выражений,
токенов и с помощью парсинга.

# Ошибки парсинга

Важной способностью каждого парсера является обработка ошибок, т.к. парсер не
должен падать только из-за одной ошибки, а должен "понимать", что это за ошибка
и парсить код дальше. Он должен возвращать минимальное количество ошибок
отображать наиболее понятное описание. Тут можно вспомнить про C++ с кучей
...
с ними справляется из-за универсальности.
...
рефлексии и ключевого слова `dynamic`. Суть его в том, что логика обхода узлов
по-умолчанию прописывается не в каждом методе `Visit`, а в одном методе
`VisitChildren`, который извлекает свойства для каждой ноды и производит их обход.
Интересным моментом является и динамическая диспетчеризация с использованием
...
такого гиганского визитора на несколько файлов.
...
древовидных структур и другие вещи: [Сериализация, отображение, сравнение и обход древовидных структур в .NET](https://github.com/KvanTTT/Articles/blob/tree-structures-serialization-comparison-and-mapping-on-net/Tree-Structures-Serialization-Comparison-and-Mapping-on-NET/Russian.md). А реализация находится в вышеупомянутом
проекте [TreeProcessing.NET](https://github.com/KvanTTT/TreeProcessing.NET).
...
<!-- TODO: возможно про оптимизации дописать -->
